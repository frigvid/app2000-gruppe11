 ...
 * The FAQ modal.
 *
 * This modal is used to create and edit FAQ items.
 *
 * @author frigvid
 * @created 2024-04-20
 * @param isAuthoring Whether the user is authoring or not.
 * @param faqId The ID of the FAQ item to edit.
...
export default function FaqModal({
	...
	/**
	 * Fetches the FAQ item if a FAQ ID is provided.
	 *
	 * @author frigvid
	 * @created 2024-04-20
	 */
			...
			<>
				<div>
					<h2 className="text-center font-semibold text-2xl">
						{t("faq.creator.modal.label")}
					</h2>
					<div className="flex flex-col w-full space-y-2">
						<Tooltip title={t("faq.creator.modal.title.tooltip")}>
							<label className="font-medium">
								{t("faq.creator.modal.title.label")}<span className="text-red-500">*</span>
							</label>
						</Tooltip>
						<input
							type="text"
							placeholder={t("faq.creator.modal.title.placeholder")}
							className="border border-gray-300 p-2 rounded"
							onChange={(e) => setTitle(e.target.value)}
						/>
						<Tooltip title={t("faq.creator.modal.summary.tooltip")}>
							<label className="font-medium">
								{t("faq.creator.modal.summary.label")}<span className="text-red-500">*</span>
							</label>
						</Tooltip>
						<TextField
							placeholder={t("faq.creator.modal.summary.placeholder")}
						...
						/>
						<label className="font-medium">
							{t("faq.creator.modal.content.label")}
						</label>
						<TextField
							placeholder={t("faq.creator.modal.content.placeholder")}
				...
				<div>
					<ConfirmBeforeAction
						confirmMessage={t("faq.creator.modal.save.confirmation")}
						onConfirm={async () => {
					...
					>
						...
						>
							{t("faq.creator.modal.save.button")}
						</button>
					</ConfirmBeforeAction>
				</div>
			</>
	...
	function editorButton() {
		return (
			<Tooltip title={t("faq.editor.info.tooltip")}>
				<IconButton
					onClick={() => {
						openModal();
					}}
	...
	function editorContent() {
		return (
			<>
				<div>
					<h2 className="text-center font-semibold text-2xl">
						{t("faq.editor.modal.label")}
					</h2>
					<div className="flex flex-col w-full space-y-2">
						<Tooltip title={t("faq.editor.modal.title.tooltip")}>
							<label className="font-medium">
								{t("faq.editor.modal.title.label")}<span className="text-red-500">*</span>
							</label>
						</Tooltip>
						<input
							type="text"
							placeholder={t("faq.editor.modal.title.placeholder")}
							className="border border-gray-300 p-2 rounded"
							value={title}
							onChange={(e) => setTitle(e.target.value)}
						/>
						<Tooltip title={t("faq.editor.modal.summary.tooltip")}>
							<label className="font-medium">
								{t("faq.editor.modal.summary.label")}<span className="text-red-500">*</span>
							</label>
						</Tooltip>
						<TextField
							placeholder={t("faq.editor.modal.summary.placeholder")}
							className="border border-gray-300 p-2 rounded h-24 whitespace-pre-wrap"
							multiline
							rows={2.5}
							value={summary}
						...
						/>
						<label className="font-medium">
							{t("faq.editor.modal.content.label")}
						</label>
						<TextField
							placeholder={t("faq.editor.modal.content.placeholder")}
							className="w-full border border-gray-300 p-2 rounded h-24 whitespace-pre-wrap"
							multiline
							rows={9}
							variant="outlined"
				...
				<div>
					<ConfirmBeforeAction
						confirmMessage={t("faq.editor.modal.save.confirmation")}
						onConfirm={async () => {
							const {error} = await supabase
								.from('faq')
								.update({
					...
					>
						...
						>
							{t("faq.editor.modal.save.button")}
						</button>
					</ConfirmBeforeAction>
				</div>
			</>
